...
class MESHWorkflow(object):
    ...
    observations : dict of {str: dict}, optional
        Dictionary containing observation data for model calibration and
        validation. See the `Notes` section for more details.
    ...
    observations : xarray.Dataset
        Observations dataset for model calibration and validation. It is
        generated by the `init_observations` method.
    ...
    init_observations(return_ds=False)
        Initialize the observations dataset for MESH.
    save(output_dir)
    ...
    def __init__(
        ...
        forcing_to_units: Optional[Dict[str, str]] = mesh_forcing_units_default,
        forcing_local_attrs: Optional[Dict[str, str]] = forcing_local_attrs_default,
        forcing_global_attrs: Optional[Dict[str, str]] = forcing_global_attrs_default,
        ddb_local_attrs: Optional[Dict[str, str]] = ddb_local_attrs_default,
        ddb_global_attrs: Optional[Dict[str, str]] = ddb_global_attrs_default,
        ddb_min_values: Optional[Dict[str, float]] = mesh_drainage_database_minimums_default,
        ddb_to_units: Optional[Dict[str, str]] = mesh_drainage_database_units_default,
        settings: Dict[str, str] = None,
        gru_dim: Optional[str] = 'NGRU',
        hru_dim: Optional[str] = 'subbasin',
        outlet_value: Optional[int] = -9999,
    ) -> None:
        """
        ...
        Notes
        -----
        - The `observations` parameter is an optional dictionary that can
          contain observation data for model calibration and validation.
        """
    ...
    def run(
        ...
        running a MESH hydrological model. This method, technically, runs the
        following methods in order: `init()`, `init_ddb()`, `init_forcing()`,
        `init_class()`, `init_hydrology()`, and `init_options()`. If
        observations are provided, it also processes them
        via `init_observations()`. Once this method is called, the MESHWorkflow
        instance will have all necessary attributes populated for saving 
        (using `.save()`) or further processing.
    ...
    def init(
        ...
        # assigning a progress tracking variable
        self._progress = ['init']
    ...
    def init_ddb(
        ...
        """
        # Since `self.init()` needs to be implemented before this method,
        # we check if it has been done
        if 'init' not in self._progress:
            raise RuntimeError(
                "`init()` method must be called before `init_observations()`."
            )
        ...
        # assign progress tracking variable
        self._progress.append('init_ddb')
    ...
    def init_forcing(
        ...
        # assign progress tracking variable
        self._progress.append('init_forcing')
    ...
    def init_class(
        ...
        # assign progress tracking variable
        self._progress.append('init_class')
    ...
    def init_hydrology(
        ...
        # assign progress tracking variable
        self._progress.append('init_hydrology')
    ...
    def init_options(
        ...
        # assign progress tracking variable
        self._progress.append('init_options')
    ...
    def init_observations(
        ...
        A list of variable names can be found in the following:
         - https://mesh-model.atlassian.net/wiki/spaces/USER/pages/6390056/Variable+List

        The location key contains a dictionary with latitude and longitude of the
        observation point.
        ...
        ----------
        return_ds : bool, optional
            If True, return the observations as an xarray.Dataset.
            If False (default), assign to self.observations and return None.

        Returns
        -------
        None or xarray.Dataset
            If `return_ds` is True, returns the observations as an
            xarray.Dataset. Otherwise, assigns to self.observations and returns None.
        """
        # Since `self.init()` needs to be implemented before this method,
        # we check if it has been done
        if 'init' not in self._progress:
        ...
        # For each observation in self._obs, check the location, and add
        # a Rank equivalent (based on the sub-basin the point location falls into)
        # then build an xarray.Dataset object
    ...
    def save(
        ...
        """
        # check if all init methods have been called
        required_inits = [
            'init',
            'init_ddb',
            'init_forcing',
        ...
        for init in required_inits:
            if init not in self._progress:
                raise RuntimeError(
                    f"`{init}()` method must be called before `save()`."
                    " If you want to save any component individually, "
                    "please save returned objects manually."
                )
